...
def graph_flat_walk(positions, geometry):
    fig, ax = plt.subplots(nrows=1, ncols=1)
    ax.set_facecolor('#f5f8fc')
    plt.plot(positions[:, 0], positions[:, 1], linewidth=0.3, c='black', zorder=1)
    plt.scatter(positions[0, 0], positions[0, 1], s=40, c='orange', marker='X', zorder=2)
...
def graph_walk_on_surface(positions, geometry, surface_parameterization):
    u, v = sym.var('u v')
